...
namespace Fcg.Users.Application.Features.Admin.Commands.PromoverUserParaAdmin
{
    public class PromoverUserParaAdminCommandHandler : IRequestHandler<PromoverUserParaAdminCommand, UserResponse>
    {
        ...
        {
            _logger.LogInformation("[UserAPI] Iniciando processo de promoção de usuário para administrador. UserId: {UserId}", request.Id);
            ...
            if (User.Perfil.Equals(TipoUser.Administrador))
            {
                _logger.LogWarning("[UserAPI] Falha na promoção. O usuário já é um administrador. UserId: {UserId}", request.Id);
            ...
            _logger.LogWarning("[UserAPI] [AUDITORIA] Usuário {TargetUserId} promovido a Administrador pelo operador {AdminId}.", request.Id, request.IdOperador);
